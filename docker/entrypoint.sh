...
function configure_ssh_authentification {
...
function customize_fdroid {
  ...
  if [ -f "$CONFIG_EXTERNAL_PATH/$CONFIG_PY" ]; then
    ...
    if [ -f "$CONFIG_EXTERNAL_PATH/$CONFIG_IN_PY" ]; then
      ...
      python3 "$HTML_INTERNAL_PATH/$CONFIG_IN_PY"
    else
...
function update_fdroid
{
  ...
  c# d "$HTML_INTERNAL_PATH" && fdroid update -c && fdroid update && cd -
}


# prepare system
configure_ssh_authentification
configure_keystore
# customize_fdroid
